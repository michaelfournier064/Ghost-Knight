...
func _ready() -> void:
	...
	# ── Hook signals with Callables ────────────────────────────────
	master_slider.value_changed.connect(
		Callable(self, "_on_master_slider_value_changed")
	)
	returnMainMenu.pressed.connect(
		Callable(self, "_on_return_main_menu_button_pressed")
	)
	returnToGame.pressed.connect(
		Callable(self, "_on_return_to_game_button_pressed")
	)
	saveChanges.pressed.connect(
		Callable(self, "_on_save_changes_pressed")
	)
